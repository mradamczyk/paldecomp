...
int minPalFactFICI(const std::string &t, std::function<char(char)> f, int minLength) {
	std::cerr << "FICI(" << minLength <<")" << std::endl;
	int n = t.size() - 1;
...
int minPalFactN2(const std::string &t, std::function<char(char)> f, int minLength) {
	std::cerr << "BRUTE(" << minLength <<")" << std::endl;
	int n = t.size() - 1;
...
int main(int argc, char **argv) {
	...
	std::function<int(std::string, std::function<char(char)>, int)> minPalFact = brute ? minPalFactN2 : minPalFactFICI;
	...
	while (std::cin >> t) {
		...
		for (int i = 1; i <= 5; ++i)
			std::cout << minPalFact(s, f, i) << std::endl;
	}
	return 0;
}
